...
// Dependencies
const { open, writeFile, readFile, truncate, unlink, rename } = require('fs/promises');
...
lib.moveFile = async (oldDir, newDir, file) => {
  const oldFileDir = lib.baseDir + oldDir + '/' + file + '.json';
  const newFileDir = lib.baseDir + newDir + '/' + file + '.json';

  try {
    const newPath = await rename(oldFileDir, newFileDir);
    return newPath;
  } catch (err) {
